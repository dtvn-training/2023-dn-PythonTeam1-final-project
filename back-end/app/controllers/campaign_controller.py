...
def select_a_campaign(campaign_id: str, token: str, db: Session):
    ...
    campaign = db.query(campaign_model.Campaign).options(load_only(*campaign_selected_columns)).filter(
        campaign_model.Campaign.user_id == user_id, campaign_model.Campaign.campaign_id == campaign_id, campaign_model.Campaign.delete_flag == False).first()
