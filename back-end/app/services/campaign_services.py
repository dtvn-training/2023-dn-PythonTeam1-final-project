...
def create_a_campaign_service(
    campaign: campaign_schemas.CampaignInfo, user_id: str, db: Session
):
    try:
        campaign_dict = campaign.dict()
        campaign_dict["user_id"] = user_id
        campaign_dict["start_date"] = convertStrToDatetime(campaign_dict["start_date"])
        campaign_dict["end_date"] = convertStrToDatetime(campaign_dict["end_date"])
        ...
        validate_campaign_fields(
            campaign_dict["name"],
            campaign_dict["status"],
            campaign_dict["budget"],
            campaign_dict["bid_amount"],
            campaign_dict["start_date"],
            campaign_dict["end_date"],
        )
        validate_creative_fields(
            creative_dict["title"],
            creative_dict["description"],
            creative_dict["img_preview"],
            creative_dict["url"],
        )
...
def select_a_campaign_service(campaign_id: str, user_id: str, db: Session):
    ...
    creative = (
        ...
        .filter(
            creative_model.Creative.campaign_id == campaign_id,
            creative_model.Creative.delete_flag == False,
        )
        .first()
    )
    if not campaign:
...
def select_number_of_pages_service(user_id: str, db: Session):
    RECORDS_EACH_PAGE = 3
    number_of_records = (
        db.query(campaign_model.Campaign.campaign_id)
        .filter(
            campaign_model.Campaign.user_id == user_id,
            campaign_model.Campaign.delete_flag == False,
        )
...
def select_a_page_campaign(page: int, user_id: str, db: Session):
    ...
    list_campaign = (
        ...
        .order_by(
            desc(campaign_model.Campaign.status), campaign_model.Campaign.created_at
        )
        .limit(RECORDS_EACH_PAGE)
    ...
    if not list_campaign:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Not Found any Campaigns"
        )
    ...
    for campaign in list_campaign:
        ...
        creative = (
            db.query(creative_model.Creative)
            .filter(
                creative_model.Creative.campaign_id == campaign.campaign_id,
                creative_model.Creative.delete_flag == False,
            )
            .first()
        )
...
def update_a_campaign_service(
    campaign: campaign_schemas.CampaignUpdate, user_id: str, db: Session
):
    campaign_record = (
        db.query(campaign_model.Campaign)
        .filter(
    ...
    creative_record = (
        db.query(creative_model.Creative)
        .filter(creative_model.Creative.campaign_id == campaign_record.campaign_id)
        .first()
    )
    # Update data
    try:
        ...
        validate_campaign_fields(
            ...
            campaign_record.bid_amount,
            campaign_record.start_date,
            campaign_record.end_date,
        )
        validate_creative_fields(
            creative_record.title,
            creative_record.description,
            creative_record.img_preview,
            creative_record.url,
        )
        db.commit()
        ...
        HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Campaign isn't updated"
        )


def delete_campaigns_service(
    campaigns: campaign_schemas.CampaignDelete, user_id, db: Session
):
    ...
    try:
        for campaign_id in campaigns.campaign_ids:
            ...
            creative_record = (
                db.query(creative_model.Creative)
                .filter(
                    creative_model.Creative.campaign_id == campaign_record.campaign_id
                )
                .first()
            )
            if not campaign_record:
...
def validate_campaign_fields(
    name, user_status, budget, bid_amount, start_date, end_date
):
    # name
    if not isinstance(name, str) or len(name) > 200:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Invalid campaign name"
        )
    # status
    ...
    if not isinstance(budget, int):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Invalid budget"
        )
    if budget <= 0:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Budget is greater than 0"
        )

    # bid_amount
    if not isinstance(bid_amount, int):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Invalid bid amount"
        )
    if bid_amount <= 0:
        raise HTTPException(
    ...
    if not isinstance(start_date, datetime):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Invalid start date"
        )
    # end_date
    if not isinstance(end_date, datetime):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Invalid end date"
        )
...
def validate_creative_fields(title, description, img_preview, url):
    ...
    if not isinstance(title, str) or len(title) > 200:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Invalid creative title"
        )
    # description
    ...
    if not isinstance(url, str) or len(url) > 200:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Invalid creative final url"
        )
